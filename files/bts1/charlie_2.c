...
void purger_listing(int nb_listing_a_conserver);
...
int main(int argc, char ** argv) {
    ...
    char chemin_racine[256] = "."; 
    int nb_listing_a_conserver = -1;
    
    if (argc > 1) {
        strcpy(chemin_racine, argv[1]);
    }

    if (argc == 3) {
        nb_listing_a_conserver = atoi(argv[2]);
    }
    if (argc == 4) {
        nb_listing_a_conserver = atoi(argv[3]);
    }
    ...
    generer_listing(chemin_racine, listing_courant, &taille_listing_courant);
    sauvegarder_listing(listing_courant, taille_listing_courant);

    if (nb_listing_a_conserver != -1) {
        purger_listing(nb_listing_a_conserver);
    }
...
void purger_listing(int nb_listing_a_conserver) {
    struct dirent ** fichiers;
    int nb_fichiers = 0;
    nb_fichiers = scandir(".", &fichiers, filtrer_fichiers_data, alphasort);
    struct tm tm;
