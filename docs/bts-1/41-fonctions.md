...
??? success "Solution"
    ...
    int main() {
        ...
        while(1) {
            chrono();
            getchar(); // La pause : on attend une frappe au clavier...
        }
    ...
    void chrono() {
        ...
        // Si c'est le premier appel, on passe à la suite
        if (timestamp == 0) {
            puts("Premier appel !");
        }
        // Si ce n'est pas le premier appel, on calcule et affiche le temps écoulé depuis le dernier appel
        else {
            printf("Temps écoulé : %lld\n", time(NULL) - timestamp);
        }
