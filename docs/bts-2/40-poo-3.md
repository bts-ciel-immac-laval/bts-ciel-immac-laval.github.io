...
1. Le programme bugue à l'affichage...
...
5.  La méthode `mousePressEvent()` est-elle accessible dans la classe `RobotHead` ? 
    Pourquoi ?

    ??? success "Solution"
        
        Oui car `mousePressEvent()` est définie en `protected` dans `QGraphicsItem` dont hérite `RobotHead` par l'intermédiaire de `RobotPart` et `QGraphicsObject`.
        
6.  La méthode `connect()` est-elle accessible dans la classe `RobotHead` ? 
    Pourquoi ?
...
7.  La méthode `connect()` est-elle accessible dans la classe `ColorItem` ? 
    Pourquoi ?
...
8.  Dans robot.cpp, à quoi sert la ligne 61 ?
    
    ??? success "Solution"
        
        Elle permet de passer des arguments aux constructeurs des classes héritées ou des attributs.
        
9.  Quelle méthode faut-il surcharger pour modifier l'apparence d'un `QGraphicsObject` ?
    
    ??? success "Solution"
        
        Il faut surcharger la méthode `QGraphicsItem::paint()`**.**
        
10. Modifier le programme pour que le robot ait une tête carrée.
...
11. Déposer une couleur sur certains membres est compliqué, modifier le programme pour ralentir l'animation.
...
12. Une image de tête apparaît lors de certains glisser-déposer, quand ?
...
13. La couleur des cercles sur les côtés est générée aléatoirement, où cela est-il codé ?
...
14. Supprimer la génération aléatoire des couleurs pour la remplacer par un set de couleurs fixes à définir.
